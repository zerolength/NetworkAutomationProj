...
class Subnet (): #create bridge when bridge is true
    def __init__ (self, nsname, bridge, ip, cidr, gw, dhcp_range):
        ...
        #subprocess.run(['sudo','ip','link','add','name',bridges['name'] + 'brdg','type','bridge'])
...
class Router ():
    ...
    def add_net(self,subnetEdge,subnetCore):
        ...
        subprocess.run(["sudo","ip","netns","exec",self.rname,"ip","route","add",subnetEdge.ip+subnetEdge.cidr,"via",broadcast])
